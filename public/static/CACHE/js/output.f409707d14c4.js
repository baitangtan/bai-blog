...
resetBtn.addEventListener('click',async()=>{const taskId=taiyiLocalStorage.getKeyData('taskId');if(taskId){try{await fetch(`/api/ai/tasks/${taskId}/cancel`,{method:'POST'});}catch(e){}}
taiyiLocalStorage.resetStorage();window.location.href='/taiyi/';});})();;(()=>{'use strict';document.addEventListener('DOMContentLoaded',()=>{document.addEventListener('infoInputModuleReady',init,{once:true});});async function init(){const tempId=new URLSearchParams(window.location.search).get('temp_id');if(tempId){await restoreFromTempCase(tempId);return;}
