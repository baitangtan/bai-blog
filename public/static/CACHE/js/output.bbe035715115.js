...
async function resetAll(){if(await messageModule.confirm({content:'是否要重置所有内容？'})){const taskId=qimenLocalStorage.getKeyData('taskId');if(taskId){try{await fetch(`/api/ai/tasks/${taskId}/cancel`,{method:'POST'});}catch(e){}}
initUrl();qimenJieguaAsync?.resetRuntime?.();qimenLocalStorage.clearStorage();qimenLocalStorage.initStorage();await inputModule.init();['name','gender','birthday','identity','question'].forEach(function(id){const el=document.getElementById(id);if(!el)return;el.value='';el.setAttribute('value','');});await qimenPaipan.init();initChatDisplay();initBtns();resetAppraiseState();resetAgentPanel();await qimenPaipan.initInputsStorage();qimenUI.updateTips(qimenUtils.DEFAULT_TIPS_TEXT,false);messageModule.info({content:'重置成功！'});}}
