...
async function resetAll(){if(await messageModule.confirm({content:'是否要重置所有内容？'})){initUrl();qimenJieguaAsync?.resetRuntime?.();qimenLocalStorage.clearStorage();qimenLocalStorage.initStorage();['name','gender','birthday','identity','question'].forEach(function(id){const el=document.getElementById(id);if(!el)return;el.value='';el.setAttribute('value','');});await inputModule.init();await qimenPaipan.init();initChatDisplay();initBtns();resetAppraiseState();resetAgentPanel();await qimenPaipan.initInputsStorage();qimenUI.updateTips(qimenUtils.DEFAULT_TIPS_TEXT,false);messageModule.info({content:'重置成功！'});}}
